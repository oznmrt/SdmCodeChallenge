using System;
using System.Collections.Generic;

#nullable disable

namespace Invoice.Entities.Concrete
{
    public partial class Lg00101Cscard
    {
        public int Logicalref { get; set; }
        public short? Doc { get; set; }
        public short? Currstat { get; set; }
        public int? Ourbankref { get; set; }
        public string Portfoyno { get; set; }
        public string Serino { get; set; }
        public string Bankname { get; set; }
        public string Specode { get; set; }
        public string Cyphcode { get; set; }
        public string City { get; set; }
        public string Owing { get; set; }
        public string Kefil { get; set; }
        public string Muhabir { get; set; }
        public short? Branch { get; set; }
        public DateTime? Duedate { get; set; }
        public DateTime? Setdate { get; set; }
        public double? Stamp { get; set; }
        public double? Amount { get; set; }
        public short? Trcurr { get; set; }
        public double? Trrate { get; set; }
        public double? Trnet { get; set; }
        public double? Reportrate { get; set; }
        public double? Reportnet { get; set; }
        public short? Riskupdate { get; set; }
        public short? Devir { get; set; }
        public short? Inuse { get; set; }
        public int? Extenref { get; set; }
        public short? CapiblockCreatedby { get; set; }
        public DateTime? CapiblockCreadeddate { get; set; }
        public short? CapiblockCreatedhour { get; set; }
        public short? CapiblockCreatedmin { get; set; }
        public short? CapiblockCreatedsec { get; set; }
        public short? CapiblockModifiedby { get; set; }
        public DateTime? CapiblockModifieddate { get; set; }
        public short? CapiblockModifiedhour { get; set; }
        public short? CapiblockModifiedmin { get; set; }
        public short? CapiblockModifiedsec { get; set; }
        public double? Collreprate { get; set; }
        public double? Colltrrate { get; set; }
        public short? Cancelled { get; set; }
        public short? Lineexctyp { get; set; }
        public short? Textinc { get; set; }
        public short? Siteid { get; set; }
        public short? Recstatus { get; set; }
        public int? Orglogicref { get; set; }
        public int? Wfstatus { get; set; }
        public string Bnbranchno { get; set; }
        public string Bnaccountno { get; set; }
        public string Deptaddr1 { get; set; }
        public string Deptaddr2 { get; set; }
        public string Deptcity { get; set; }
        public string Deptcitycode { get; set; }
        public string Deptcountry { get; set; }
        public string Deptcountrycode { get; set; }
        public string Deptpostcode { get; set; }
        public string Depttelnrs1 { get; set; }
        public string Depttelnrs2 { get; set; }
        public string Deptfaxnr { get; set; }
        public string Depttown { get; set; }
        public string Depttowncode { get; set; }
        public string Deptdistrict { get; set; }
        public string Deptdistrictcode { get; set; }
        public short? Opstat { get; set; }
        public short? Printcnt { get; set; }
        public string Newserino { get; set; }
        public int? Projectref { get; set; }
        public string Telcodes1 { get; set; }
        public string Telcodes2 { get; set; }
        public string Faxcode { get; set; }
        public short? Affectcollatrl { get; set; }
        public int? Collatrollref { get; set; }
        public int? Collatcardref { get; set; }
        public short? Affectrisk { get; set; }
        public double? Giroreprate { get; set; }
        public double? Girotrrate { get; set; }
        public string Orglogoid { get; set; }
        public double? Giroamount { get; set; }
        public double? Girorepnet { get; set; }
        public short? Usegirorate { get; set; }
        public string Taxnr { get; set; }
        public string Iban { get; set; }
        public string Tradinggrp { get; set; }
        public short? Useraisedval { get; set; }
        public double? Raisedvalamount { get; set; }
        public double? Raisedvalrepnet { get; set; }
        public int? Salesmanref { get; set; }
        public int? Subduration { get; set; }
        public short? Degactive { get; set; }
        public short? Degcurr { get; set; }
        public double? Degcurrrate { get; set; }
        public short? Ciro { get; set; }
        public string Kefil2 { get; set; }
        public string Guid { get; set; }
        public int? Offerref { get; set; }
        public DateTime? Printdate { get; set; }
        public short? Status { get; set; }
        public int? Bncreref { get; set; }
    }
}
