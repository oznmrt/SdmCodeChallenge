using System;
using System.Collections.Generic;

#nullable disable

namespace Invoice.Entities.Concrete
{
    public partial class Lg00201Eximwhtran
    {
        public int Logicalref { get; set; }
        public int? Eximwhfcref { get; set; }
        public short? Trcode { get; set; }
        public int? Itemref { get; set; }
        public DateTime? Date { get; set; }
        public int? Ftime { get; set; }
        public short? Factory { get; set; }
        public short? Sourceindex { get; set; }
        public short? Destindex { get; set; }
        public int? Uomref { get; set; }
        public int? Usref { get; set; }
        public double? Amount { get; set; }
        public double? Price { get; set; }
        public double? Total { get; set; }
        public short? Trcurr { get; set; }
        public double? Trrate { get; set; }
        public double? Reportrate { get; set; }
        public string Specode { get; set; }
        public string Lineexp { get; set; }
        public short? Lineno { get; set; }
        public short? Siteid { get; set; }
        public short? Recstatus { get; set; }
        public int? Orglogicref { get; set; }
        public double? Avamount { get; set; }
        public double? Uinfo1 { get; set; }
        public double? Uinfo2 { get; set; }
        public double? Uinfo3 { get; set; }
        public double? Uinfo4 { get; set; }
        public double? Uinfo5 { get; set; }
        public double? Uinfo6 { get; set; }
        public double? Uinfo7 { get; set; }
        public double? Uinfo8 { get; set; }
        public double? Grossuinfo1 { get; set; }
        public double? Grossuinfo2 { get; set; }
        public short? Linetype { get; set; }
        public int? Prevlineref { get; set; }
        public short? Prevlineno { get; set; }
        public short? Detline { get; set; }
        public short? Prcurr { get; set; }
        public double? Prrate { get; set; }
        public double? Prprice { get; set; }
        public int? Srcaccref { get; set; }
        public int? Srccenterref { get; set; }
        public int? Dstaccref { get; set; }
        public int? Dstcenterref { get; set; }
        public double? Impprice { get; set; }
        public double? Unitexpense { get; set; }
        public short? Impcurr { get; set; }
        public double? Imprate { get; set; }
        public short? Textinc { get; set; }
        public int? Variantref { get; set; }
        public int? Faregref { get; set; }
        public double? Adjpriceufrs { get; set; }
        public double? Adjreppriceufrs { get; set; }
        public string Specode2 { get; set; }
        public int? Projectref { get; set; }
        public int? Eximfileref { get; set; }
        public short? Eximprocnr { get; set; }
        public int? Origincntrref { get; set; }
        public short? Status { get; set; }
        public int? Srcexpaccref { get; set; }
        public int? Srcexpcenref { get; set; }
        public int? Dstexpaccref { get; set; }
        public int? Dstexpcenref { get; set; }
    }
}
