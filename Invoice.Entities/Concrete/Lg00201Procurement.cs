using System;
using System.Collections.Generic;

#nullable disable

namespace Invoice.Entities.Concrete
{
    public partial class Lg00201Procurement
    {
        public int Logicalref { get; set; }
        public int? Prodordref { get; set; }
        public int? Polineref { get; set; }
        public int? Displineref { get; set; }
        public short? Invennr { get; set; }
        public int? Itemref { get; set; }
        public int? Uomref { get; set; }
        public double? Amount { get; set; }
        public int? Mainitemref { get; set; }
        public int? Mainuomref { get; set; }
        public double? Mainamount { get; set; }
        public short? Procuretype { get; set; }
        public int? Ficheref { get; set; }
        public int? Fichelineref { get; set; }
        public int? Clcardref { get; set; }
        public int? Plnstficheref { get; set; }
        public int? Plnstlineref { get; set; }
        public double? Uinfo1 { get; set; }
        public double? Uinfo2 { get; set; }
        public double? Uinfo3 { get; set; }
        public double? Uinfo4 { get; set; }
        public double? Uinfo5 { get; set; }
        public double? Uinfo6 { get; set; }
        public double? Uinfo7 { get; set; }
        public double? Uinfo8 { get; set; }
        public short? Itemalter { get; set; }
        public short? Itemtemp { get; set; }
        public double? Mainuinfo1 { get; set; }
        public double? Mainuinfo2 { get; set; }
        public double? Mainuinfo3 { get; set; }
        public double? Mainuinfo4 { get; set; }
        public double? Mainuinfo5 { get; set; }
        public double? Mainuinfo6 { get; set; }
        public double? Mainuinfo7 { get; set; }
        public double? Mainuinfo8 { get; set; }
        public double? Grossuinfo1 { get; set; }
        public double? Grossuinfo2 { get; set; }
        public double? Grossmainuinfo1 { get; set; }
        public double? Grossmainuinfo2 { get; set; }
        public short? Converted { get; set; }
        public int? Variantref { get; set; }
        public int? Mainvariantref { get; set; }
        public short? Prodordtyp { get; set; }
    }
}
