using System;
using System.Collections.Generic;

#nullable disable

namespace Invoice.Entities.Concrete
{
    public partial class Lg00101Demandline
    {
        public int Logicalref { get; set; }
        public int? Demandficheref { get; set; }
        public int? Itemref { get; set; }
        public int? Clientref { get; set; }
        public double? Amount { get; set; }
        public double? Meetamnt { get; set; }
        public double? Cancamount { get; set; }
        public int? Uomref { get; set; }
        public int? Usref { get; set; }
        public double? Uinfo1 { get; set; }
        public double? Uinfo2 { get; set; }
        public double? Uinfo3 { get; set; }
        public double? Uinfo4 { get; set; }
        public double? Uinfo5 { get; set; }
        public double? Uinfo6 { get; set; }
        public double? Uinfo7 { get; set; }
        public double? Uinfo8 { get; set; }
        public short? Meettype { get; set; }
        public DateTime? Procuredate { get; set; }
        public short? Sourceindex { get; set; }
        public short? Branch { get; set; }
        public short? Department { get; set; }
        public short? Factorynr { get; set; }
        public int? Bommasterref { get; set; }
        public int? Bomrevref { get; set; }
        public string Specode { get; set; }
        public string Lineexp { get; set; }
        public short? Status { get; set; }
        public short? Siteid { get; set; }
        public short? Recstatus { get; set; }
        public int? Orglogicref { get; set; }
        public int? Wfstatus { get; set; }
        public int? Paydefref { get; set; }
        public short? Linetype { get; set; }
        public string Cyphcode { get; set; }
        public short? Cpstflag { get; set; }
        public short? Detline { get; set; }
        public int? Prevlineref { get; set; }
        public short? Prevlineno { get; set; }
        public short? Lineno { get; set; }
        public string Username { get; set; }
        public DateTime? Fichedate { get; set; }
        public int? Mrplineref { get; set; }
        public int? Mrpheadref { get; set; }
        public short? Altitemuse { get; set; }
        public short? Mrpheadtype { get; set; }
        public short? Ordpeguse { get; set; }
        public double? Ordpegamount { get; set; }
        public double? Price { get; set; }
        public int? Procuretime { get; set; }
        public short? Invuseparam { get; set; }
        public int? Prodordref { get; set; }
        public int? Polineref { get; set; }
        public int? Displineref { get; set; }
        public int? Plnstfcref { get; set; }
        public int? Plnstlref { get; set; }
        public short? Plnficheper { get; set; }
        public short? Realsrcindex { get; set; }
        public int? Accountref { get; set; }
        public int? Centerref { get; set; }
        public int? Projectref { get; set; }
        public int? Crsaccountref { get; set; }
        public int? Crscenterref { get; set; }
        public int? Crsprojectref { get; set; }
        public short? Prcurr { get; set; }
        public double? Prprice { get; set; }
        public int? Variantref { get; set; }
        public short? Meetwithstock { get; set; }
        public short? Fichetype { get; set; }
        public string Guid { get; set; }
        public short? Prodordtyp { get; set; }
        public short? Bomtype { get; set; }
        public int? Prclistref { get; set; }
        public int? Frclineref { get; set; }
    }
}
